...
**Data seeding is not optional.** `DataSeeder` (`CommandLineRunner`) runs on every backend boot when `app.seed.enabled=true` (default) and seeds the admin account (`admin@prepplushub.com`, password from `ADMIN_SEED_PASSWORD` or a random one generated and logged once at boot), demo products, and `PlatformSettings` — but only if they don't already exist. Because it runs inside `SpringApplication.run`, a Mongo connection failure at boot fails the *entire* application context (this also means `BackendApplicationTests.contextLoads` requires a live MongoDB — see Known Issues).

**OTP falls back to dev-mode automatically.** `AuthService` emails the OTP via `EmailService` (Spring Mail / SMTP, configured through `MAIL_HOST`/`MAIL_USERNAME`/`MAIL_PASSWORD`/`MAIL_FROM`). If SMTP isn't configured — or sending fails — it falls back to printing the OTP to the console (`[PrepplusHub OTP] email => code`) and including it in the API response, so registration still works end-to-end without SMTP set up. `app.otp.expose-in-response` is now `false` by default; it's the fallback path that actually controls whether dev-mode kicks in, not this flag alone.
